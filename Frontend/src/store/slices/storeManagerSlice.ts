...
import type { StoreManager, CreateStoreManagerPayload } from '../../interfaces/StoreManagerTypes/storeManager';
...
export const fetchStoreManagers = createAsyncThunk<StoreManager[]>(
  ...
  async (_, { rejectWithValue }) => {
    try {
      ...
      return rejectWithValue(error.response?.data?.message || 'Failed to fetch store managers');
    }
  }
);

export const createStoreManager = createAsyncThunk<StoreManager, CreateStoreManagerPayload>(
  ...
  async (payload, { rejectWithValue }) => {
    try {
      ...
      return rejectWithValue(error.response?.data?.message || 'Failed to create store manager');
    }
  }
);
...
// ─── Selectors ────────────────────────────────────────────────────────────────
export const selectStoreManagers = (state: RootState) => state.storeManagers.managers;
export const selectStoreManagerStatus = (state: RootState) => state.storeManagers.status;
export const selectStoreManagerError = (state: RootState) => state.storeManagers.error;
